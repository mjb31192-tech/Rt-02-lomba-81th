      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
          ...
          <motion.div
            ...
            className="relative max-w-lg w-full bg-slate-50 rounded-2xl overflow-hidden shadow-2xl border border-slate-100 flex flex-col max-h-[90vh] z-10"
          >
            {/* Modal Header */}
            <div className="bg-gradient-to-r from-red-600 to-red-700 px-5 py-4 flex items-center justify-between text-white shadow-xs">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-display font-black text-sm uppercase tracking-wider leading-none">
                    Laporan Iuran Saya
                  </h3>
              ...
              <button
                type="button"
                onClick={onClose}
                className="p-1.5 hover:bg-white/10 text-white/80 hover:text-white rounded-lg transition-all cursor-pointer hover:rotate-90 duration-200"
              >
                <X size={18} />
              </button>
            </div>
                    ...
                    {kkData.riwayat.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {kkData.riwayat.map((item, idx) => (
                          <div 
                            key={item.id || idx} 
                            className="flex items-center justify-between p-2 rounded-lg bg-gray-50 border border-gray-150 text-xs hover:bg-gray-100 transition-all font-medium text-gray-800"
                          >
            ...
            <div className="bg-white px-5 py-3.5 border-t border-gray-100 flex justify-end">
              <button
                type="button"
                onClick={onClose}
                className="bg-red-600 hover:bg-red-700 text-white text-xs font-bold px-5 py-2 rounded-xl transition-all shadow-xs cursor-pointer active:scale-95 hover:shadow-md hover:shadow-red-100"
              >
                Selesai &amp; Mengerti
              </button>
            </div>
